...
// Return landmarks that are within the range of the particle
inline std::vector<LandmarkObs> find_prediction(const Map map) {
	std::vector<LandmarkObs> output;

	for(int i = 0; i < map.landmark_list.size(); i++) {
		Map::single_landmark_s single_landmark = map.landmark_list[i];
		LandmarkObs landmark_converted;
		landmark_converted.x = (double) single_landmark.x_f;
		landmark_converted.y = (double) single_landmark.y_f;
		landmark_converted.id = single_landmark.id_i;
...
inline std::vector<LandmarkObs> transform_observations(const std::vector<LandmarkObs> observations,
																									double x,
																									double y,
																									double theta) {
	...
	for(int i = 0; i < observations.size(); i++) {
		...
		obs_transformed.x = obs_x * cos(theta) - obs_y * sin(theta) + x;
		obs_transformed.y = obs_x * sin(theta) + obs_y * cos(theta) + y;

		obs_transformed_list.push_back(obs_transformed);
	}
